...
function VyHub:server_data_ready()
    VyHub:msg(string_format("I am server %s in bundle %s.", VyHub.server.name, VyHub.server.serverbundle.name))

    VyHub.ready = true

    hook_Run("vyhub_ready")
end
...
hook_Add("vyhub_api_ready", "vyhub_main_vyhub_api_ready", function ()
    VyHub.API:get("/server/%s", { VyHub.Config.server_id }, nil, function(code, result) 
        ...
        VyHub:msg(string_format("Could not find server with id %s", VyHub.Config.server_id))
    end)

    VyHub:get_frontend_url()
end)

hook_Add("vyhub_api_failed", "vyhub_main_vyhub_api_failed", function ()
